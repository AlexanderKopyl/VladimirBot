...
$perfect_candidate = "Изучи моё предложение для зарплатных клиентов Сбербанка или ВТБ
Детали по ссылке на сайте http://vladimir-berlizov.ru/perfect_sberbank_client/";
$perfect_candidate_ipoteka = "Изучи моё предложение для клиентов с официальной работой.
...
$ipoteka_answer_6_2 = "НЕТ. Я безработный.";
...
$ipoteka_instr = file_get_contents("ipoteka_instruction.txt");
$ipoteka_consult = file_get_contents("ipoteka_consultations.txt");
$ipoteka_online_sem = file_get_contents("ipoteka_internet_workshops.txt");
$ipoteka_tur_help = file_get_contents("ipoteka_turnkey_help.txt");
...
Посмотри моё обучающее видео с советами, как получить кредит в сложных случаях.";
$ipoteka_text3 = "Банки требуют, чтобы бизнес официально работал больше 1 года. 
...
Посмотри моё обучающее видео с советами, как получить кредит в сложных случаях. ";
$ipoteka_text4 = "Банки требуют предоставление НЕ нулевых и НЕ убыточных деклараций. 
...
Посмотри моё обучающее видео с советами, как получить кредит в сложных случаях. ";
$ipoteka_text5 = "90% банков не одобряют ипотеку БЕЗ взноса. Здесь есть 2 решения: 
1) Если ты планируешь покупать новостройку, некоторые застройщики предлагают возможность ипотеки без взноса. Это делается через схему с завышением цены квартиры и подделку кассовых приходных ордеров. Такая опция есть не у всех застройщиков, уточняй у менеджеров строительных фирм. 
Ни в коем случае не спрашивай об этом в банках. Это палево.  
2) Ипотека по двум документам с первоначальным взносом 40% или 50%. Сначала накопи эту сумму, а только потом подавай заявки в банки. 
Во-первых, в большинстве банков бесполезно завышать цену квартиры на вторичном рынке и прятать взнос через расписку от продавца. Даже если ты договоришься с оценщиком и он завысит цену квартиры, банки перепроверяют все отчёты и могут уменьшить оценку. Ты потеряешь деньги на взятку оценщику и время на сорванную сделку. 
И второй момент - часто в программах по двум документам есть требование внести деньги на счёт в банке.";
$ipoteka_text6_1 = "90% банков не одобряют ипотеку людям с неофициальной работой.

Здесь есть два решения: 
1) Ипотека по двум документам с первоначальным взносом 40% или 50%. Сначала накопи эту сумму, а только потом подавай заявки в банки. 

Во-первых, в большинстве банков бесполезно завышать цену квартиры на вторичном рынке и прятать взнос через расписку от продавца. Даже если ты договоришься с оценщиком и он завысит цену квартиры, банки перепроверяют все отчёты и могут уменьшить оценку. Ты потеряешь деньги на взятку оценщику и время на сорванную сделку. 

И второй момент - часто в программах по двум документам есть требование внести деньги на счёт в банке.

2) Посмотри моё обучающее видео с советами, как получить ипотеку с неофициальной работой.
";
$ipoteka_text6_2 = "Банки не одобряют ипотеку безработным. 
...
2) Посмотри моё обучающее видео с советами, как получить ипотеку с неофициальной работой.";
$ipoteka_text8 = "90% банков требуют, чтобы ты работал на последнем месте работы минимум 6 месяцев. Иначе - отказ!

Подожди, пока у тебя будет нужный стаж - 6 месяцев или посмотри моё обучающее видео с советами, как получить кредит в сложных случаях.";
$ipoteka_text9 = "90% банков требуют, чтобы за последние 5 лет у тебя был непрерывный стаж от 12 месяцев. Не должно быть перерыва между работами больше, чем 30 дней. Иначе - отказ!
 
Подожди, пока у тебя будет нужный стаж - 12 месяцев или посмотри моё обучающее видео с советами, как получить кредит в сложных случаях.";
$ipoteka_text10 = "90% банков не одобряют ипотеку без взноса. 

Здесь есть 2 решения: 
1) Если ты планируешь покупать новостройку, некоторые застройщики предлагают такую возможность. Это делается через схему с завышением цены квартиры и подделку кассовых приходных ордеров. Это есть не у всех застройщиков, уточняй у менеджеров строительных фирм. В банках не спрашивай. Это палево.  
...
//        sendMessage($telegram,$chat_id,$first_step_no_answer);
    ...
    elseif ($text == $ipoteka_answer_1){
        ...
        sendQuestion($telegram,$chat_id,$ipoteka_question_2,[[$ipoteka_answer_2],[$ipoteka_answer_2_1]]);
...
//        sendMessage($telegram,$chat_id,$business_no_answer);
        ...
        sendInlineButton($chat_id,$ipoteka_text3,"Смотреть видео","http://vladimir-berlizov.ru/unqualified_sme_loan_candidate/");
//        sendMessage($telegram,$chat_id,$business_no_answer);
    }elseif ($text == $ipoteka_answer_4){
        putAnswerToFile($name,'ipoteka_answer_4',true);
        sendQuestion($telegram,$chat_id,$ipoteka_question_5,[[$ipoteka_answer_5],[$ipoteka_answer_5_1],]);
    } elseif ($text == $ipoteka_answer_4_1){
        putAnswerToFile($name,'ipoteka_answer_4',false);
        sendInlineButton($chat_id,$ipoteka_text4,"Смотреть видео","http://vladimir-berlizov.ru/unqualified_sme_loan_candidate/");
//        reply_html($telegram,$chat_id,$ipoteka_html);
        ...
        sendMessage($telegram,$chat_id,$ipoteka_text5);
        //sendFunctionService
        sendService($telegram,$chat_id,[
            ["I) Инструкции"],
            ["II)  Консультации"],
    ...
    } elseif ($text == $ipoteka_answer_6){
        putAnswerToFile($name,'ipoteka_answer_6',true);
        sendQuestion($telegram,$chat_id,$ipoteka_question_7,[[$ipoteka_answer_7],[$ipoteka_answer_7_1]]);
    } elseif ($text == $ipoteka_answer_6_1){;
        putAnswerToFile($name,'ipoteka_answer_6',false);
        sendInlineButton($chat_id,$ipoteka_text6_1,"Смотреть видео","http://vladimir-berlizov.ru/unqualified_cash_loan_candidate/");
    } elseif ($text == $ipoteka_answer_6_2){;
        putAnswerToFile($name,'ipoteka_answer_6',false);
        sendInlineButton($chat_id,$ipoteka_text6_2,"Смотреть видео","http://vladimir-berlizov.ru/unqualified_cash_loan_candidate/");
    }elseif ($text == $ipoteka_answer_7){
        putAnswerToFile($name,'ipoteka_answer_7',true);
        sendMessage($telegram,$chat_id,$perfect_candidate);
    } elseif ($text == $ipoteka_answer_7_1){
        putAnswerToFile($name,'ipoteka_answer_7',false);
        sendQuestion($telegram,$chat_id,$ipoteka_question_8,[[$ipoteka_answer_8],[$ipoteka_answer_8_1]]);
    } elseif ($text == $ipoteka_answer_8){
        putAnswerToFile($name,'ipoteka_answer_8',true);
        sendQuestion($telegram,$chat_id,$ipoteka_question_9,[[$ipoteka_answer_9],[$ipoteka_answer_9_1]]);
    }elseif ($text == $ipoteka_answer_8_1){
        putAnswerToFile($name,'ipoteka_answer_8',false);
        sendInlineButton($chat_id,$ipoteka_text8,"Смотреть видео","http://vladimir-berlizov.ru/");
    }elseif ($text == $ipoteka_answer_9){
        putAnswerToFile($name,'ipoteka_answer_9',false);
        sendMessage($telegram,$chat_id,$perfect_candidate_ipoteka);
        sendQuestion($telegram,$chat_id,$ipoteka_question_10,[[$ipoteka_answer_10],[$ipoteka_answer_10_1]]);
    }elseif ($text == $ipoteka_answer_9_1){
        putAnswerToFile($name,'ipoteka_answer_9',false);
        sendInlineButton($chat_id,$ipoteka_text9,"Смотреть видео","http://vladimir-berlizov.ru/unqualified_cash_loan_candidate/");
    }elseif ($text == $ipoteka_answer_10){
        putAnswerToFile($name,'ipoteka_answer_10',false);
        sendService($telegram,$chat_id,[
            ["I) Инструкции"],
            ["II)  Консультации"],
            ["III) Помощь. Под ключ"],
    ...
    }elseif ($text == $ipoteka_answer_10_1){
        putAnswerToFile($name,'ipoteka_answer_10',false);
        sendMessage($telegram,$chat_id,$ipoteka_text10);
        sendService($telegram,$chat_id,[
            ["I) Инструкции"],
            ["II)  Консультации"],
            ["III) Помощь. Под ключ"],
    ...
    elseif ($text == "I) Инструкции"){
        reply_html($telegram,$chat_id,$ipoteka_instr);
    }elseif ($text == "II)  Консультации"){
        reply_html($telegram,$chat_id,$ipoteka_consult);
    }elseif ($text == "III) Помощь. Под ключ"){
        reply_html($telegram,$chat_id,$ipoteka_tur_help);
    }elseif ($text == "IV) Интернет-Семинары"){
        reply_html($telegram,$chat_id,$ipoteka_online_sem);
    }
